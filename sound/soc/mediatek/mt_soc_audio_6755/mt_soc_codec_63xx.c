...
#define AW8736_MODE5 /*it depends on THD, range: 1.5 ~ 2.0w*/ \
					do { \
						mt_set_gpio_out(GPIO_SPEAKER_EN_PIN, GPIO_OUT_ONE); \
						udelay(GAP); \
						mt_set_gpio_out(GPIO_SPEAKER_EN_PIN, GPIO_OUT_ZERO); \
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	if (enable) {
...
#ifndef CONFIG_MTK_SPEAKER
		pr_warn("Ext_Speaker_Amp_Change ON set GPIO\n");
		mt_set_gpio_mode(GPIO_SPEAKER_EN_PIN, GPIO_MODE_00);	/* GPIO117: DPI_D3, mode 0 */
		mt_set_gpio_pull_enable(GPIO_SPEAKER_EN_PIN, GPIO_PULL_ENABLE);
		mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN, GPIO_DIR_OUT);	/* output */
		mt_set_gpio_out(GPIO_SPEAKER_EN_PIN, GPIO_OUT_ZERO);	/* low disable */
		udelay(1000);
		mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN, GPIO_DIR_OUT);	/* output */
		...
		AW8736_MODE5;
#else
		mt_set_gpio_out(GPIO_SPEAKER_EN_PIN, GPIO_OUT_ONE);	/* high enable */
#endif
		...
		/* mt_set_gpio_mode(GPIO_SPEAKER_EN_PIN, GPIO_MODE_00); //GPIO117: DPI_D3, mode 0 */
		mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN, GPIO_DIR_OUT);	/* output */
		mt_set_gpio_out(GPIO_SPEAKER_EN_PIN, GPIO_OUT_ZERO);	/* low disbale */
		udelay(500);
#endif
		pr_debug("Ext_Speaker_Amp_Change OFF-\n");
	}
...
static int mt6331_codec_probe(struct snd_soc_codec *codec)
{
	...
	if ((data[0] == 0) && (data[1] > 40 && data[1] < 54)) {
		/* 0.505v : rework version -- use pin_vowclk(GPIO 54) */
#if defined(CONFIG_MTK_LEGACY)
		ret = GetGPIO_Info(4, &pin_extspkamp, &pin_mode_extspkamp);
		if (ret < 0) {
			pr_err("Ext_Speaker_Amp_Change GetGPIO_Info FAIL!!!\n");
			return;
		}
		pr_warn("Get AW8736 SHDN IO %u from DTS (REWORK)\n", (pin_extspkamp - 0x80000000));
#else
		pin_extspkamp = -1;
