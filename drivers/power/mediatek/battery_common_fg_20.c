...
static void battery_update(struct battery_data *bat_data)
{
	...
	if(BMT_status.charger_type==4 && bat_data->BAT_BatteryAverageCurrent ==0 && bat_data->BAT_STATUS != POWER_SUPPLY_STATUS_FULL)
	{
			...
			bat_data->BAT_BatteryAverageCurrent =(AC_CHARGER_CURRENT / 100*29+ICharging)/30;
	}
}
...
void mt_battery_GetBatteryData(void)
{
	...
	BMT_status.ZCV = ZCV;
	bat_temp_Avg=BMT_status.temperature;
	chr_typeNum=BMT_status.charger_type;
	batteryIndex++;
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		{
			battery_log(BAT_LOG_CRTI, "[BATTERY]bat_vol over voltage !!\r\n");
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if (BMT_status.temperature > HIGH_TEMPERATURE_NOTIFY) {
...
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
	else if (BMT_status.temperature < LOW_TEMPERATURE_NOTIFY) {
		g_BatteryNotifyCode |= 0x0020;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too low)\n",
				    BMT_status.temperature);
	}
...
#if defined(CONFIG_TOUCHSCREEN_MTK_FT5436_ZAL1518)
extern void tpd_usb_plugin(u8 plugin);
#endif
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Get_META_BAT_VOL:
...
#if 1//  add for identify the flash ID ny meta tool
					msdc_emmc_id_check();
					user_data_addr = (int *)arg;
					ret = copy_from_user(adc_in_data, user_data_addr, 8);
					adc_out_data[0] = BMT_status.bat_vol;//for the battery voltage
					battery_log(BAT_LOG_CRTI, "**** unlocked_ioctl : BAT_VOL:%d\n", adc_out_data[0]);	
					battery_log(BAT_LOG_CRTI, "****g_emmc_raw_cid 00 :0x%x\n", g_emmc_raw_cid[0]);	 
					battery_log(BAT_LOG_CRTI, "****g_emmc_raw_cid 01 :0x%x\n", g_emmc_raw_cid[1]);	 
					battery_log(BAT_LOG_CRTI, "****g_emmc_raw_cid 02 :0x%x\n", g_emmc_raw_cid[2]);	 
					battery_log(BAT_LOG_CRTI, "****g_emmc_raw_cid 03 :0x%x\n", g_emmc_raw_cid[3]);	 
					...
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 01 after :0x%x\n", temp_buf[0]);	
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 02 after :0x%x\n", temp_buf[1]);	
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 03 after :0x%x\n", temp_buf[2]);  
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 04 after :0x%x\n", temp_buf[3]);	
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 05 after :0x%x\n", temp_buf[4]);	
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 06 after :0x%x\n", temp_buf[5]);	 
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 07 after :0x%x\n", temp_buf[6]);  
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 08 after :0x%x\n", temp_buf[7]);	
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 09 after :0x%x\n", temp_buf[8]); 
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 10 after :0x%x\n", temp_buf[4]<<8);		   
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid 11 after :0x%x\n", temp_buf[4]<<8 | temp_buf[5]);					
					//old:adc_out_data[1] = (temp_buf[4]<<8 | temp_buf[5]);
					//get the totally 9bytes ID information;
					adc_out_data[1] = g_emmc_raw_cid[0];//high 4bytes
					adc_out_data[2] = g_emmc_raw_cid[1];//mid 4bytes
					adc_out_data[3] = temp_buf[8];//the last byte
					battery_log(BAT_LOG_CRTI,"****g_emmc_raw_cid adc_out_data[1] :0x%x\n", adc_out_data[1]);   
					ret = copy_to_user(user_data_addr, adc_out_data, 13);//sizeof(adc_out_data)); // 
		#endif
		//user_data_addr = (int *)arg;
		//ret = copy_from_user(adc_in_data, user_data_addr, 8);
	...
	case Get_META_BAT_SOC:
		//printk("****xxxx unlocked_ioctl : BAT_VOL:%d\n", adc_out_data[0]);
		//printk("****xxxx unlocked_ioctl : BMT_status.UI_SOC2:%d\n", BMT_status.UI_SOC2);
		//printk("****xxxx unlocked_ioctl : BMT_status.SOC:%d\n", BMT_status.SOC);
		//printk("****xxxx unlocked_ioctl : BMT_status.UI_SOC:%d\n", BMT_status.UI_SOC);
		user_data_addr = (int *)arg;
		ret = copy_from_user(adc_in_data, user_data_addr, 8);
...
//liuxiong add 20131122 start
			...
			if (*user_data_addr == 2) {
					...
					battery_log(BAT_LOG_CRTI, "[BATTERY] runing test disable charging!!\n");
			}
			else if (*user_data_addr == 1 || *user_data_addr == 3) {
					icharging_enable = KAL_TRUE;
					g_charger_should_not_open = KAL_FALSE;					/* open charger */
					battery_log(BAT_LOG_CRTI, "[BATTERY] runing test enable charging!!\n");
			}
