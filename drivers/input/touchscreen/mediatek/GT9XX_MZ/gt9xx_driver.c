...
extern int ltr559_read_ps(void);//add by wangyang
...
{  
    ...
    //sscanf(buf, "%x", &value);
    //gesture_ctrl = value;
    //value1 = (value>>8)&0x000000ff;
    //value2 = (value>>24)&0x000000ff;
	/*
	...
	*/
    ...
    GTP_INFO("yili: >> buff[0]= %d buff[1]= %d buff[2]= %d buff[3]= %d\n",buff[0],buff[1],buff[2],buff[3]);
    value1 = buff[0] ;
    value2 = buff[2] ; 
    ...
    GTP_INFO("value = %x, value1 = %x, value2 = %x gesture_mask = %x\n",value,value1,value2,gesture_mask);
    GTP_INFO("gesture_three_byte_one=%x,gesture_three_byte_two=%x,gesture_three_byte_three=%x,gesture_three_byte_four=%x\n",gesture_three_byte_one,gesture_three_byte_two,gesture_three_byte_three,gesture_three_byte_four);
    return count;
}
...
s32 gtp_read_version(struct i2c_client *client, u16 *version)
{
	...
	printk(KERN_ERR "gtp_read_version vid = %d\n", *version);
	...
	//add for lsensor need GT915L chip id 20160510 by HQ
	if((buf[2]=='9')&&(buf[3]=='1')&&(buf[4]=='5')&&(buf[5]=='L')){
		ctp_chip_id = 37212;
		GTP_INFO("lsensor read 915L read chip id sucess ctp_chip_id = %d\n",ctp_chip_id);
	}else{
		ctp_chip_id = -1;
		GTP_ERROR("lsensor read 915L read chip id fail ctp_chip_id= %d\n",ctp_chip_id);
	}
	//add end
...
s32 gtp_read_lcm_color_info(struct i2c_client *client, u8 *lcm_color_info)
{
    ...
    printk("[gt9xx] white =55 black=0xaa *lcm_color_info =0x%x\n",*lcm_color_info);
    ctp_chip_colour = 1;
...
static int tpd_irq_registration(void)
{
	...
	if(node){
		...
		gtp_touch_irq = irq_of_parse_and_map(node, 0);
		GTP_INFO("Device gtp_int_type = %d!", int_type);
		if (!int_type)	//EINTF_TRIGGER
		{
			ret = request_irq(gtp_touch_irq, (irq_handler_t)tpd_eint_interrupt_handler, EINTF_TRIGGER_RISING, "TOUCH_PANEL-eint", NULL);
		...
		{
			ret = request_irq(gtp_touch_irq, (irq_handler_t)tpd_eint_interrupt_handler, EINTF_TRIGGER_FALLING, "TOUCH_PANEL-eint", NULL);
	...
	GTP_INFO("[%s]irq:%d, debounce:%d-%d:", __FUNCTION__, gtp_touch_irq, ints[0], ints[1]);
	return ret;
}
#endif
...
#endif
#if GTP_GESTURE_WAKEUP
    input_set_capability(tpd->dev, EV_KEY, KEY_GESTURE);//wangyang add
#endif
...
static irqreturn_t tpd_eint_interrupt_handler(unsigned irq, struct irq_desc *desc)
{
	...
	//disable_irq_nosync(gtp_touch_irq);
	//irq_flag--;
	//GTP_INFO("disable irq_flag=%d",irq_flag);
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
		...
		if((gesture_ps)&&(gesture_onoff)){
			 gesture_data = 0x00;
                        // clear 0x814B
                        doze_buf[2] = 0x00;
                        gtp_i2c_write(i2c_client_point, doze_buf, 3);
		GTP_INFO("gesture_onoff = %d gesture_ps=%d \n",gesture_onoff,gesture_ps);
		}
		//printk("GTP far wait --------------2\n");
        tpd_flag = 0;
        TPD_DEBUG_SET_TIME;
	   ...
	   { 
	     ...
	     { 
			...
			else
			{
                ...
                else if ( (doze_buf[2] == 0xAA) || (doze_buf[2] == 0xBB) ||(doze_buf[2] == 0xAB) || (doze_buf[2] == 0xBA) )
                {
