...
#include <linux/proc_fs.h>   //proc file use by likai
#ifdef CONFIG_COMPAT
...
struct tpd_driver_t *g_tpd_drv;//add proc  by likai begin 
#define PROC_CTP_INFO "driver/ctp_info" //by likai
...
//static struct tpd_driver_t *g_tpd_drv = NULL;   //bylikai
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	proc_create(PROC_CTP_INFO, 0, NULL, &fctp_proc_fops1);//by likai add /proc/driver/ctp
#ifdef CONFIG_HAS_EARLYSUSPEND
#ifndef CONFIG_MTK_FPGA
	MTK_TS_early_suspend_handler.suspend = g_tpd_drv->suspend;
	MTK_TS_early_suspend_handler.resume = g_tpd_drv->resume;
