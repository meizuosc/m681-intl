...
//#define FTS_COMMON_DBG(fmt, arg...)	printk(" %s. line: %d.  "fmt"\n", __FUNCTION__, __LINE__, ##arg)
#else
#define FTS_COMMON_DBG(fmt, args...) do{}while(0)
//#define FTS_COMMON_DBG(fmt, arg...) (void)(0)
#endif
/*monitor enable and wait time*/
#define  FTS_MONITOR_EN			1//0:disable, 1: enable
#define MONITOR_WAIT_TIME 			200//ms 触发监视动作的间隔时间（单位：ms）

#define  FTS_MONITOR_DEBUG			1//0:disable, 1: enable
/*jugde distance*/
#define JUGDE_DISTANCE_EN	 				0    //0=close, 1=open, default 0
#define DISTANCE_BETWEEN_TWO_POINTS		100//设置两点之间的最小距离
...
extern struct i2c_client *fts_i2c_client;
//extern int f_drv_up_event_count;
//extern int f_drv_other_event_count;
...
//	int ret = 0;
	...
	uljiffies=msecs_to_jiffies(MONITOR_WAIT_TIME+20);
	FTS_COMMON_DBG("");
	...
	{		
		...
		do
		{
			...
			if(MONITOR_WAIT_TIME < iDeltaTime)
			{
...
static int fts_parse_touch_data(char *readbuf, unsigned char readlen)
{
	...
	else
	{
		...
		{
			id = ((readbuf[ADDR_ID_POS+i*EVERY_POINT_LEN])>>4); //y轴高字节第 1-5bit
			//st_monitor.current_data.point_id[id] = id;
			if(id < FTS_MAX_POINT_ID)
			{
				st_monitor.current_data.event[id] = ((readbuf[ADDR_EVENT_POS+i*EVERY_POINT_LEN])>>6);  //x轴高字节2bit, event
...
extern void tpd_down(int x, int y, int p);
extern void tpd_up(int x, int y);
...
static ssize_t fts_touch_count_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch(buff_data)
		{
		case 0:
			ret = fts_write_reg(client, 0x90, 0x00);
			if(ret < 0)goto WRITE_ERR;
			msleep(10);
			ret = fts_write_reg(client, 0x91, 0x00);
			if(ret < 0)goto WRITE_ERR;
			msleep(10);
			ret = fts_write_reg(client, 0x92, 0x00);
			if(ret < 0)goto WRITE_ERR;
			msleep(10);
			ret = fts_write_reg(client, 0x93, 0x00);
			if(ret < 0)goto WRITE_ERR;
			msleep(10);
			ret = fts_read_reg(client, 0x90, &fw_up_event_l);
			if(ret < 0)goto WRITE_ERR;	
			msleep(10);
			ret = fts_read_reg(client, 0x91, &fw_up_event_h);
			if(ret < 0)goto WRITE_ERR;
			msleep(10);
			ret = fts_read_reg(client, 0x92, &fw_other_event_l);
			if(ret < 0)goto WRITE_ERR;
			msleep(10);
			ret = fts_read_reg(client, 0x93, &fw_other_event_h);
			if(ret < 0)goto WRITE_ERR;	
			
			//g_drv_up_event_count = 0;
			//g_drv_other_event_count = 0;
			//f_drv_up_event_count = 0 ;
			//f_drv_other_event_count = 0;
			
			FTS_COMMON_DBG("Clear touch count. From FW: up = %d, down&contact = %d. From Driver: up = %d, down&contact = %d.Output Driver: up = %d, down&contact = %d",
				fw_up_event_l + (fw_up_event_h<<8),
				fw_other_event_l + (fw_other_event_h<<8),
				//g_drv_up_event_count,
				//g_drv_other_event_count,
				//f_drv_up_event_count,
				//f_drv_other_event_count
				);
			break;
		case 1:
			ret = fts_read_reg(client, 0x90, &fw_up_event_l);
