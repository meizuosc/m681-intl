...
static ssize_t show_alsps(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if(g_alsps_name)
        ret_value = sprintf(buf, "AlSPS name  :%s\n",g_alsps_name); 	
         ...
         #if defined(CONFIG_HQ_ZAL826_PROJECT)
         ret_value = sprintf(buf, "AlSPS name  :AP3426H\n"); 
        #else
...
static ssize_t show_gsensor(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if(g_gsensor_name)
        ...
        #if defined(CONFIG_HQ_ZAL826_PROJECT)
         ret_value = sprintf(buf, "GSensor name:LSM6DS0\n"); 
        #else
...
static ssize_t show_msensor(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if(g_msensor_name)
        ...
        #if defined(CONFIG_HQ_ZAL826_PROJECT)
         ret_value = sprintf(buf, "MSensor name:AK09911C\n"); 
        #else
...
static ssize_t show_gyro(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if(g_Gyro_name)
        ...
        #if defined(CONFIG_HQ_ZAL826_PROJECT)
         ret_value = sprintf(buf, "Gyro  name :LSM6DS0\n"); 
        #else
...
static ssize_t show_main_otp(struct device *dev,struct device_attribute *attr, char *buf)
{
   ...
   // {
    //   if(hw_info_main_otp.otp_valid)
     //       ret_value = sprintf(buf, "Main OTP:Vendor 0x%0x, Version %d-%d-%d, Lens 0x%0x, Vcm 0x%0x\n",
    //hw_info_main_otp.vendor_id,hw_info_main_otp.year,hw_info_main_otp.month,hw_info_main_otp.day,hw_info_main_otp.lens_id,hw_info_main_otp.vcm_id); 	
  //      else
...
static ssize_t store_main_otp(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);	
    return size;
}
...
static ssize_t show_sub_otp(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //        ret_value = sprintf(buf, "SUB OTP:Vendor 0x%0x, Version %d-%d-%d, Lens 0x%0x, Vcm 0x%0x\n",
            	//hw_info_sub_otp.vendor_id,hw_info_sub_otp.year,hw_info_sub_otp.month,hw_info_sub_otp.day,hw_info_sub_otp.lens_id,hw_info_sub_otp.vcm_id); 	
      //  else
...
static ssize_t store_sub_otp(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);	
    return size;
}

char *fingerprint_name  = "gf318m";
static ssize_t show_fingerprint(struct device *dev,struct device_attribute *attr, char *buf)
{
    int ret_value = 1;
    #if defined(CONFIG_FINGERPRINT)
    if(fingerprint_name)
        ret_value = sprintf(buf, "FingerPrint name  :%s\n",fingerprint_name); 
    #else
    ret_value = sprintf(buf, "FingerPrint name  :Not support FingerPrint\n"); 	
    #endif
    return ret_value;
}

static ssize_t store_fingerprint(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);	
    return size;
}
...
static DEVICE_ATTR(04_flash, 0664, show_flash, store_flash);
...
static DEVICE_ATTR(14_main_otp, 0664, show_main_otp, store_main_otp);
static DEVICE_ATTR(15_sub_otp, 0664, show_sub_otp, store_sub_otp);
static DEVICE_ATTR(16_fingerprint, 0664, show_fingerprint, store_fingerprint);
...
{	
    ...
    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_14_main_otp)) != 0) goto exit_error;
    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_15_sub_otp)) != 0) goto exit_error;
    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_16_fingerprint)) != 0) goto exit_error;
    ...
    //memset(&hw_info_sub_otp, 0, sizeof(hw_info_sub_otp));
...
static int HardwareInfo_driver_remove(struct platform_device *dev)
{
    ...
    device_remove_file(&(dev->dev), &dev_attr_14_main_otp);
    device_remove_file(&(dev->dev), &dev_attr_15_sub_otp);
    device_remove_file(&(dev->dev), &dev_attr_16_fingerprint);
