...
int _esd_check_config_handle_vdo(cmdqRecHandle handle)
{
	...
	/*	mutex sof wait*/
	ddp_mutex_set_sof_wait(dpmgr_path_get_mutex(primary_get_dpmgr_handle()), handle, 0);
...
int primary_display_esd_check(void)
{
	...
	//add by fanxueliang
	if((cur_stat == DISP_ALIVE)&&(esd_flag == 1)){
		 esd_flag=0;
		 printk("fxl primary_display_esd_check");
	...
	if (primary_get_lcm()->params->dsi.customization_esd_check_enable == 0) {
		...
		if (GPIO_EINT_MODE == mode) {
			...
			MMProfileLogEx(ddp_mmp_get_events()->esd_extte, MMProfileFlagPulse, primary_display_is_video_mode(), mode);
