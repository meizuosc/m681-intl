...
static int acc_write_block(int *buf, int len);
static int acc_read_block(int *buf, int len);

#define ACC_BASE_ADDR (3*1024*1024)
#define ACC_POS (9*64+20)
#define STORE_FILE      "/dev/block/platform/mtk-msdc.0/by-name/proinfo"
static int first_enable_flag = 0;

static int lsm6ds3_accel_discard_data_flag = 1;
static int lsm6ds3_accel_discard_data_count =0;
...
static unsigned long lsm6ds3_init_flag_test;	/*initial in module_init     = 0; initial state*/
...
static int acc_read_block(int *data, int len)
{
    struct file *f;
    mm_segment_t fs;
    int ret;
    ...
    if (IS_ERR(f)) {
        GSE_ERR("read_block open file %s error!\n", STORE_FILE);
        return -1;
    }

    fs = get_fs(); set_fs(KERNEL_DS);
    f->f_pos=ACC_BASE_ADDR+ACC_POS;
    ret = f->f_op->read(f, buffer, 16, &f->f_pos);
    set_fs(fs);
    filp_close(f, NULL);
    if (ret < 0 ) {
        GSE_ERR("%s: read_block read file %s error!\n", __func__, STORE_FILE);
        ret = -1;
    }

	data[0] = *((int *)(buffer+0));
	data[1] = *((int *)(buffer+6));
	data[2] = *((int *)(buffer+12));
	GSE_ERR("read_block x = %d, y = %d, z = %d\n",data[0], data[1], data[2]);
...
static int acc_write_block(int *buf, int len)
{
	...
	*((int *)(buffer+12)) = buf[2];	
    ...
    if (IS_ERR(f)) {
		GSE_ERR("%s: write_block open file %s error!\n", __func__, STORE_FILE);
		return -1;
    }
    ...
    GSE_ERR("set x = %d, y = %d, z = %d\n",buf[0], buf[1], buf[2]);
    res = f->f_op->write(f, buffer, 16, &f->f_pos);
    set_fs(fs);
    filp_close(f, NULL);
    if ( res < 0){
		GSE_ERR("write_block error ret =%d\n", res);
		return -1;
    }
    return 0;
}
...
static int LSM6DS3_acc_Reset_Axis_Calibration(int axis)
{
	struct lsm6ds3_i2c_data *obj = obj_i2c_data;

	if(axis == LSM6DS3_AXIS_X){
		memset(&obj->cali_sw[LSM6DS3_AXIS_X], 0x00, sizeof(obj->cali_sw[LSM6DS3_AXIS_X]));
	}
	else if(axis == LSM6DS3_AXIS_Y){
		memset(&obj->cali_sw[LSM6DS3_AXIS_Y], 0x00, sizeof(obj->cali_sw[LSM6DS3_AXIS_Y]));
	}
	else if(axis == LSM6DS3_AXIS_Z){
		memset(&obj->cali_sw[LSM6DS3_AXIS_Z], 0x00, sizeof(obj->cali_sw[LSM6DS3_AXIS_Z]));
	}

	return 0;
}

static int LSM6DS3_acc_Read_Axis_Calibration(int *dat, int axis)
{
    struct lsm6ds3_i2c_data *obj = obj_i2c_data;

	if(axis == LSM6DS3_AXIS_X){
		*dat = obj->cvt.sign[LSM6DS3_AXIS_X]*obj->cali_sw[LSM6DS3_AXIS_X];
	}
	else if(axis == LSM6DS3_AXIS_Y){
		*dat = obj->cvt.sign[LSM6DS3_AXIS_Y]*obj->cali_sw[LSM6DS3_AXIS_Y];
	}
	else if(axis == LSM6DS3_AXIS_Z){
		*dat = obj->cvt.sign[LSM6DS3_AXIS_Z]*obj->cali_sw[LSM6DS3_AXIS_Z];
	}

	return 0;
}

static int LSM6DS3_acc_Write_Axis_Calibration(int dat, int axis)
{
	struct lsm6ds3_i2c_data *obj = obj_i2c_data;
	...
	if(axis == LSM6DS3_AXIS_X){
		cali = obj->cvt.sign[LSM6DS3_AXIS_X] * obj->cali_sw[LSM6DS3_AXIS_X];
		cali += dat;
		obj->cali_sw[LSM6DS3_AXIS_X] = obj->cvt.sign[LSM6DS3_AXIS_X]*cali;
	}
	else if(axis == LSM6DS3_AXIS_Y){
		cali = obj->cvt.sign[LSM6DS3_AXIS_Y] * obj->cali_sw[LSM6DS3_AXIS_Y];
		cali += dat;
		obj->cali_sw[LSM6DS3_AXIS_Y] = obj->cvt.sign[LSM6DS3_AXIS_Y]*cali;
	}
	else if(axis == LSM6DS3_AXIS_Z){
		cali = obj->cvt.sign[LSM6DS3_AXIS_Z] * obj->cali_sw[LSM6DS3_AXIS_Z];
		cali += dat;
		obj->cali_sw[LSM6DS3_AXIS_Z] = obj->cvt.sign[LSM6DS3_AXIS_Z]*cali;
	}
...
static int LSM6DS3_acc_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if (true == enable) {
        ...
        lsm6ds3_accel_discard_data_flag =1;
		lsm6ds3_accel_discard_data_count =0;
	} else {
...
static int LSM6DS3_ReadAccData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (res < 0) {
		...
		*/
	#endif
        if (lsm6ds3_accel_discard_data_flag ==1)
	{
		if(lsm6ds3_accel_discard_data_count <=4){
			 acc[LSM6DS3_AXIS_X] = 0;
			 acc[LSM6DS3_AXIS_Y] = 0;
			 acc[LSM6DS3_AXIS_Z] = 0;
			 lsm6ds3_accel_discard_data_count ++;	     	
		}    else{
			 lsm6ds3_accel_discard_data_flag = 0;	
		}		
	}
...
static int lsm6ds3_enable_nodata(int en)
{
	...
	if(first_enable_flag == 0)
	{
		first_enable_flag = 1;
		err = acc_read_block(cali_data,12);
		if(err == 0){
			priv->cali_sw[LSM6DS3_AXIS_X] = priv->cvt.sign[LSM6DS3_AXIS_X]*cali_data[LSM6DS3_AXIS_X];
			priv->cali_sw[LSM6DS3_AXIS_Y] = priv->cvt.sign[LSM6DS3_AXIS_Y]*cali_data[LSM6DS3_AXIS_Y];
			priv->cali_sw[LSM6DS3_AXIS_Z] = priv->cvt.sign[LSM6DS3_AXIS_Z]*cali_data[LSM6DS3_AXIS_Z];
			GSE_ERR("acc enable obj->cali_sw x=%d,y=%d,z=%d\n",priv->cali_sw[LSM6DS3_AXIS_X],priv->cali_sw[LSM6DS3_AXIS_Y],priv->cali_sw[LSM6DS3_AXIS_Z]);
		}else{
			GSE_ERR("acc enable read calibration data error\n");
		}
	}
...
{
	GSE_LOG("lsm6ds3_step_c_enable_significant start\n");
	int res = 0;
	struct lsm6ds3_i2c_data *priv = obj_i2c_data;
	...
	GSE_LOG("lsm6ds3_step_c_enable_significant end\n");
	return res;
}
#endif
...
static int lsm6ds3_step_c_enable_nodata(int en)
{
	...
	int err = 0;
	struct lsm6ds3_i2c_data *priv = obj_i2c_data;
	GSE_LOG("lsm6ds3_step_c_enable_nodata start!\n");
...
static int acc_do_calibration(struct i2c_client *client)
{
	int i;
	int flag = 0;
	char strbuf[LSM6DS3_BUFSIZE];
	int x, y, z, pre_x, pre_y, pre_z;
	int sum_x = 0, sum_y = 0, sum_z = 0;
	int cali[3];

	LSM6DS3_acc_ResetCalibration(client);
	for(i = 0; i < 20; i++)
	{
		LSM6DS3_ReadAccData(client, strbuf, LSM6DS3_BUFSIZE);
		sscanf(strbuf, "%x %x %x", &x, &y, &z);
		...
		{
			printk(KERN_ERR "acc_do_cali please put phone on flat surface\n");
			return -1;
		}
		else if(z < 0)
		{
			printk(KERN_ERR "acc_do_cali please let phone screen toward sky\n");
			return -1;
		}
		else if(abs(x - pre_x) > 500 || abs(y - pre_y) > 500 || abs(z - pre_z) > 500)
		{
			printk(KERN_ERR "acc_do_cali please let phone static on flat surface\n");
			return -1;
		}
		else
		{
	...
	cali[LSM6DS3_AXIS_X] = -1*(sum_x/20);
	cali[LSM6DS3_AXIS_Y] = -1*(sum_y/20);
	cali[LSM6DS3_AXIS_Z] = -1*(sum_z/20 - 9800);
	LSM6DS3_acc_WriteCalibration(client, cali);

	return 0;
}

static ssize_t acc_calibration_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	GSE_FUN();
	int ret;
	int cali_data[3] = {0};
	...
	ret = acc_do_calibration(client);

	if(ret == 0)
	{	
	    LSM6DS3_acc_ReadCalibration(client, cali_data);
		ret = acc_write_block(cali_data, 12);
		if(ret == -1){
...
static ssize_t acc_x_calibbias_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	GSE_FUN();
	int cali;

	LSM6DS3_acc_Read_Axis_Calibration(&cali, LSM6DS3_AXIS_X);
	return sprintf(buf, "%d\n",cali);
}

static ssize_t acc_y_calibbias_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	GSE_FUN();
	int cali = 0;

	LSM6DS3_acc_Read_Axis_Calibration(&cali, LSM6DS3_AXIS_Y);
	return sprintf(buf, "%d\n",cali);
}

static ssize_t acc_z_calibbias_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	GSE_FUN();
	int cali = 0;

	LSM6DS3_acc_Read_Axis_Calibration(&cali, LSM6DS3_AXIS_Z);
	return sprintf(buf, "%d\n",cali);
}

static ssize_t acc_x_offset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	GSE_FUN();
	int cali = 0;

	if (1 == sscanf(buf, "%d", &cali)){
		LSM6DS3_acc_Reset_Axis_Calibration(LSM6DS3_AXIS_X);
		LSM6DS3_acc_Write_Axis_Calibration(cali, LSM6DS3_AXIS_X);
	}
	else{
		GSE_ERR(" Sysfs Write X Axis Calibration Error!\n");
	}

	return size;
}

static ssize_t acc_y_offset_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	GSE_FUN();
	int cali = 0;

	if (1 == sscanf(buf, "%d", &cali)){
		LSM6DS3_acc_Reset_Axis_Calibration(LSM6DS3_AXIS_Y);
		LSM6DS3_acc_Write_Axis_Calibration(cali, LSM6DS3_AXIS_Y);
	}
	else{
		GSE_ERR(" Sysfs Write Y Axis Calibration Error!\n");
	}

	return size;
}

static ssize_t acc_z_offset_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	GSE_FUN();
	int cali = 0;

	if (1 == sscanf(buf, "%d", &cali)){
		LSM6DS3_acc_Reset_Axis_Calibration(LSM6DS3_AXIS_Z);
		LSM6DS3_acc_Write_Axis_Calibration(cali, LSM6DS3_AXIS_Z);
	}
	else{
		GSE_ERR(" Sysfs Write Z Axis Calibration Error!\n");
	}

	return size;
}

static DEVICE_ATTR(acc_calibration, S_IWUSR | S_IRUGO, acc_calibration_show,	 NULL);
static DEVICE_ATTR(acc_x_calibbias, S_IWUSR | S_IRUGO, acc_x_calibbias_show, NULL);
static DEVICE_ATTR(acc_y_calibbias, S_IWUSR | S_IRUGO, acc_y_calibbias_show, NULL);
static DEVICE_ATTR(acc_z_calibbias, S_IWUSR | S_IRUGO, acc_z_calibbias_show, NULL);
static DEVICE_ATTR(acc_x_offset,	S_IWUSR  | S_IRUGO, NULL, acc_x_offset_store);
static DEVICE_ATTR(acc_y_offset, 	S_IWUSR  | S_IRUGO, NULL, acc_y_offset_store);
static DEVICE_ATTR(acc_z_offset, 	S_IWUSR  | S_IRUGO, NULL, acc_z_offset_store);

static struct device_attribute *acc_calibrate_dev_attributes[] = {
...
static long lsm6ds3_acc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_READ_SENSORDATA:
			...
			GSE_ERR("LSM6DS3_DEBUG: READ_SENSORDATA\n");
			data = (void __user *) arg;
			if (data == NULL) {
				err = -EINVAL;
				break;
	...
	case GSENSOR_IOCTL_READ_OFFSET:
			...
			GSE_ERR("LSM6DS3_DEBUG: READ_OFFSET\n");
			data = (void __user *) arg;
			if (data == NULL) {
				err = -EINVAL;
				break;
	...
	case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			GSE_ERR("LSM6DS3_DEBUG: READ_RAW_DATA\n");
			data = (void __user *) arg;
			if (data == NULL) {
				err = -EINVAL;
				break;
	...
	case GSENSOR_IOCTL_SET_CALI:
			...
			GSE_ERR("LSM6DS3_DEBUG: SET_CALI\n");
			data = (void __user *)arg;
			if (data == NULL) {
	...
	case GSENSOR_IOCTL_CLR_CALI:
			...
			GSE_ERR("LSM6DS3_DEBUG: CLR_CALI\n");
			err = LSM6DS3_acc_ResetCalibration(client);
			break;

	case GSENSOR_IOCTL_GET_CALI:

			GSE_ERR("LSM6DS3_DEBUG: GET_CALI\n");
			data = (void __user *)arg;
			if (data == NULL) {
				err = -EINVAL;
				break;
...
static void acc_cali_work(struct work_struct *work)
{
	int ret;
	int cali_data[3] ={0};

	struct lsm6ds3_i2c_data *obj = (struct lsm6ds3_i2c_data *)container_of(work, struct lsm6ds3_i2c_data, cali_work);
	
		if (obj == NULL) {
			GSE_ERR("lsm6ds3_i2c_data is NULL!\n");
			return -1;
		}

		ret = acc_read_block(cali_data,12);
		if(ret == 0){
			obj->cali_sw[LSM6DS3_AXIS_X] = obj->cvt.sign[LSM6DS3_AXIS_X]*cali_data[LSM6DS3_AXIS_X];
			obj->cali_sw[LSM6DS3_AXIS_Y] = obj->cvt.sign[LSM6DS3_AXIS_Y]*cali_data[LSM6DS3_AXIS_Y];
			obj->cali_sw[LSM6DS3_AXIS_Z] = obj->cvt.sign[LSM6DS3_AXIS_Z]*cali_data[LSM6DS3_AXIS_Z];
			GSE_ERR("acc probe:obj->cali_sw x=%d,y=%d,z=%d\n",obj->cali_sw[LSM6DS3_AXIS_X],obj->cali_sw[LSM6DS3_AXIS_Y],obj->cali_sw[LSM6DS3_AXIS_Z]);
		}else{
			GSE_ERR("acc probe read calibration data error\n");
		}
}
...
static int lsm6ds3_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	acc_cal_device= kzalloc(sizeof(struct device), GFP_KERNEL);
	if (!acc_cal_device) {
			GSE_ERR("can not allocate acc_cal_device\n");
		goto exit_register_path_fail;
	}

	acc_cal_device->init_name = "acc";
	acc_cal_device->class = &meizu_class;
	acc_cal_device->release = (void (*)(struct device *))kfree;

	result = device_register(acc_cal_device);
	if (result) {
	   GSE_ERR("can not register acc_cal_device\n");
		goto exit_register_path_fail;
	}

	for (i = 0; i < ARRAY_SIZE(acc_calibrate_dev_attributes); i++) {
		result = device_create_file(acc_cal_device,  acc_calibrate_dev_attributes[i]);
		if (result)
	        break;
	}
	...
	if (result) {
	    while (--i >= 0){
	        device_remove_file(acc_cal_device, acc_calibrate_dev_attributes[i]);
	    }
		GSE_ERR("device remove file = %d\n", result);
	    goto exit_create_attr_failed;
	}
	...
	INIT_DELAYED_WORK(&obj->cali_work, acc_cali_work);
	msleep(10);
...
static int lsm6ds3_step_c_local_init(void)
{
	...
	GSE_LOG("lsm6ds3_step_c_local_init start\n");
	...
	GSE_LOG("lsm6ds3_step_c_local_init end\n");
	return 0;
...
static int __init lsm6ds3_init(void)
{
	...
	lsm6ds3_init_flag_test = 0;		/*initial in module_init   = 0; initial state*/
