                                 ...
                                 struct device_attribute *attr, char *buf) 
{
	...
	printk("gsensor name is %s by likai\n",devname);//by likai
	return snprintf(buf, PAGE_SIZE, "%s\n", devname+5); 
}
static ssize_t acc_show_sensorname(struct device* dev, 
                                 struct device_attribute *attr, char *buf) 
{
	printk("acc name is %s by likai \n",Accname);//by likai 
	//return snprintf(buf, PAGE_SIZE, "%s\n", Accname); 
	return sprintf(buf, "%s\n",Accname);
}
static struct proc_dir_entry *fts_proc_Gsensor_name=NULL;//by likai begin

static int fts_Gsensor_name_read( struct file *file, char __user *buffer, size_t count, loff_t *ppos )
{
    char *page = NULL;
    char *ptr = NULL;
    int i, len, err = -1;
...
static int acc_real_driver_init(void)
{
	...
	printk(" before acc Accname is %s by likai\n",Accname);
		...
		if (0 != gsensor_init_list[i]) {
			...
			if (0 == err) {
				ACC_LOG(" acc real driver %s probe ok\n",
					gsensor_init_list[i]->name);
				memcpy(Accname, gsensor_init_list[i]->name, strlen(gsensor_init_list[i]->name));//by likai
				break;
			}
		}
	}
	...
	printk("after acc Accname is %s by likai\n",Accname);
	return err;
}
