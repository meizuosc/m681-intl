...
int gfx1xm_power_on(struct gfx1xm_dev *gfx1xm_dev, bool onoff)
{
	...
	if(onoff) {
		if (0 == gfx1xm_dev->poweron){
			...
			mt_set_gpio_mode(gfx1xm_dev->irq_gpio, GFX1XM_IRQ_PIN_M_GPIO);
			mt_set_gpio_dir(gfx1xm_dev->irq_gpio, GPIO_DIR_IN); 
			mt_set_gpio_pull_enable(gfx1xm_dev->irq_gpio, GPIO_PULL_ENABLE);
			mt_set_gpio_pull_select(gfx1xm_dev->irq_gpio, GPIO_PULL_DOWN);
			
			/*Reset GPIO Output-low before poweron*/
			mt_set_gpio_mode(gfx1xm_dev->reset_gpio, GFX1XM_RST_PIN_M_GPIO);
			mt_set_gpio_dir(gfx1xm_dev->reset_gpio, GPIO_DIR_OUT); 
			mt_set_gpio_out(gfx1xm_dev->reset_gpio, GPIO_OUT_ZERO); 
			msleep(5);

			/*power on*/
			/*Reset GPIO Output-low before poweron*/
			mt_set_gpio_mode(GFX1XM_EXT_LDO_PIN, GFX1XM_EXT_LDO_PIN_M_GPIO);
			mt_set_gpio_dir(GFX1XM_EXT_LDO_PIN, GPIO_DIR_OUT); 
			mt_set_gpio_out(GFX1XM_EXT_LDO_PIN, GPIO_OUT_ONE); 
			...
			mt_set_gpio_pull_enable(gfx1xm_dev->irq_gpio, GPIO_PULL_DISABLE);
			msleep(1);
			
			/*Reset GPIO Output-high, GFX1XM works*/
			mt_set_gpio_out(gfx1xm_dev->reset_gpio, GPIO_OUT_ONE); 	
			msleep(60);
		}
	} else {
		if (1 == gfx1xm_dev->poweron){
			...
			mt_set_gpio_out(gfx1xm_dev->reset_gpio, GPIO_OUT_ZERO); 
			msleep(10);
			mt_set_gpio_out(GFX1XM_EXT_LDO_PIN, GPIO_OUT_ZERO); 
...
static long gfx1xm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GFX1XM_IOC_REINIT:
		mutex_lock(&gfx1xm_dev->fb_lock);
	   // mt_eint_mask(gfx1xm_dev->spi->irq);
	    disable_irq(finger_irq);
	    gfx1xm_hw_reset(gfx1xm_dev, 60);
	    //mt_eint_unmask(gfx1xm_dev->spi->irq);
	    enable_irq(finger_irq);
...
int gfx1xm_config_update(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	u8 cfg6_info_group3[] = GFX16M_CFG_GROUP3;
	...
	if(chip_version_config==0x38){		
		...
		if(!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_1, 1)) {
			...
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg6_info_group3, GFX1XM_CFG_LEN);
			printk("gfx1xm: Vendor ID not matched!!! no send\n");
			//return 0;
		}
	}
...
#error "have't define any product for fw"
	...
	gfx1xm_dev->spi->irq = GFX1XM_IRQ_NUM;
	...
	}
	{
		...
		if(node){
			...
			finger_irq = irq_of_parse_and_map(node, 0);
			
			ret = request_irq(finger_irq, gfx1xm_irq, EINTF_TRIGGER_RISING, "FINGERPRINT-eint", NULL);
			if(ret > 0){
			    ret = -1;
			    printk("fingerprint request_irq IRQ LINE NOT AVAILABLE!.");
			}
		} else {
			printk("fingerprint request_irq can not find touch eint device node!.");
			ret = -1;
		}
		printk("[%s]irq:%d", __FUNCTION__, finger_irq);
	}
