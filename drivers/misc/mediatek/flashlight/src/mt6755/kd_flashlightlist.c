...
/*==============================*/
#define FLASHLIGHT_ATTR_RO(_name) \
	DEVICE_ATTR(_name, S_IRUGO, show_##_name, NULL)
#define FLASHLIGHT_ATTR_RW(_name) \
	DEVICE_ATTR(_name, 0664 , show_##_name, store_##_name)

static mz_flash_data mzflashdata = {-1, -1, 0, 0};
...
static int flashlight_open(struct inode *inode, struct file *file)
{
	...
	mzflashdata.flash1_enable = 0;
	mzflashdata.flash2_enable = 0;
	mzflashdata.flash1_level = -1;
	mzflashdata.flash2_level = -1;
...
/* ==========================[zhangdong 2016-02-20]============================ */
extern int lm3643_torch_mode(mz_flash_data *torchData);

static ssize_t show_flash1(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	logI("flashlight show flash1_level = %d \n", mzflashdata.flash1_level);
	return scnprintf(buf, PAGE_SIZE, "%d\n", mzflashdata.flash1_level);
}

static ssize_t store_flash1(struct device *dev,
					...
					const char *buf, size_t len)
{
	...
	if (level < 0) {
		...
		mzflashdata.flash1_level = 5;
		mzflashdata.flash1_enable = 1;
	} else {
		mzflashdata.flash1_level = level;
		mzflashdata.flash1_enable = 1;
	}
	logI("flashlight store flash1_level = %d \n", mzflashdata.flash1_level);
	lm3643_torch_mode(&mzflashdata);

	return len;
}

static ssize_t show_flash2(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	logI("flashlight show flash1_level = %d \n", mzflashdata.flash2_level);
	return scnprintf(buf, PAGE_SIZE, "%d\n", mzflashdata.flash2_level);
}

static ssize_t store_flash2(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t len)
{
	int level;
	...
	if (level < 0) {
		...
		mzflashdata.flash2_level = 5;
		mzflashdata.flash2_enable = 1;
	} else {
		mzflashdata.flash2_level = level;
		mzflashdata.flash2_enable = 1;
	}
	logI("flashlight store flash2_level = %d \n", mzflashdata.flash2_level);
	lm3643_torch_mode(&mzflashdata);

	return len;
}

static FLASHLIGHT_ATTR_RW(flash1);
static FLASHLIGHT_ATTR_RW(flash2);

static struct attribute *flashlight_attributes[] = {
	&dev_attr_flash1.attr,
	&dev_attr_flash2.attr,
	NULL,
};

static umode_t flashlight_attr_is_visible(struct kobject *kobj,
...
static struct attribute_group flashlight_attribute_group = {
	.is_visible	= flashlight_attr_is_visible,
	.attrs		= flashlight_attributes
};
/* ==========================[zhangdong 2016-02-20]============================ */
...
#define FLASHLIGHT_MAJOR 242
	...
	ret = sysfs_create_group(&flashlight_device->kobj,
						&flashlight_attribute_group);
	if (ret != 0) {
