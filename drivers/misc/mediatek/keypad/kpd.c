...
#ifdef CONFIG_MTK_MEIZU_CLASS 	// add /sys/class/meizu/hall/key_hall_state for mz by 20160223
extern struct class meizu_class;
#define HALL_DEVICE_NAME  "hall"
static struct device *hall_device;
static int slid_state = 3 ;	// far to hall
#endif
...
/********************************************************************************************/
#ifdef CONFIG_MTK_MEIZU_CLASS // add /sys/class/meizu/hall/key_hall_state for mz by 20160223
static ssize_t key_hall_state_show(struct device *dev,struct device_attribute *attr, char *buf)
{
...
static void kpd_slide_handler(unsigned long data)
{
...
#ifdef CONFIG_MTK_MEIZU_CLASS	// add /sys/class/meizu/hall/key_hall_state for mz by 20160223
	if(slid){
	 slid_state = 2;	//1->2: cover(near to hall)
	}else{
	 slid_state = 3;	//0->3: no cover
	}	
#endif
	...
	if (old_state == IRQ_TYPE_EDGE_RISING)
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	if (err) {
		...
		if (!hall_device) {
				kpd_info("cannot allocate hall device\n");		 
			return -ENOMEM;
		}
	
		hall_device->init_name = HALL_DEVICE_NAME;
		hall_device->class = &meizu_class;
		hall_result = device_register(hall_device);
		if (hall_result) {
		   kpd_info("HZF cannot register hall_device\n");
			return hall_result;
		}
	 
		for (cnt = 0; cnt < ARRAY_SIZE(hall_dev_attributes); cnt++) {
			hall_result = device_create_file(hall_device,  hall_dev_attributes[i]);
			if (hall_result)
				break;
		}	
		if (hall_result) {
			while (--cnt >= 0){
				device_remove_file(hall_device,hall_dev_attributes[i]);
			}
			 kpd_info("device remove file  = %d\n", hall_result);	
