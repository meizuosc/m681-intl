...
extern void kpd_slide_eint_handler(void);	//kpd_slide_eint_handler
static int hall_irq_registration(void)
{
	...
	if(node){
		...
		ret = request_irq(hall_irq, kpd_slide_eint_handler, EINTF_TRIGGER_FALLING, "KPD_SLIDE-eint", NULL);
		if(ret > 0){
		    ret = -1;
		    printk("hall request_irq IRQ LINE NOT AVAILABLE!.");
		}
	} else {
		printk("hall request_irq can not find touch eint device node!.");
		ret = -1;
	}
	printk("[%s]irq:%d, debounce:%d-%d:", __FUNCTION__, hall_irq, ints[0], ints[1]);
	return ret;
}
#endif
...
void kpd_slide_qwerty_init(void)
{
	...
	handle = rcu_dereference(kpd_input_dev->grab);
	if (handle) {
		handler = handle->handler;
		if (strcmp(handler->name, "evdev") == 0)
			return -1;
	} else {
		list_for_each_entry_rcu(handle, &kpd_input_dev->h_list, d_node) {
			handler = handle->handler;
			if (strcmp(handler->name, "evdev") == 0) {
				evdev_flag = true;
				break;
	...
	//mt_eint_registration(KPD_SLIDE_EINT,EINTF_TRIGGER_FALLING,kpd_slide_eint_handler, false);
...
/********************************************************************/
#define CONFIG_ONEKEY_REBOOT_META_MODE
#define CONFIG_ONEKEY_REBOOT_RECOVERY_MODE
...
#ifdef CONFIG_ONEKEY_REBOOT_META_MODE
	else if (get_boot_mode() == META_BOOT) {
		pmic_set_register_value(MT6351_PMIC_RG_PWRKEY_RST_EN, 0x01);
		pmic_set_register_value(MT6351_PMIC_RG_HOMEKEY_RST_EN, 0x00);
		pmic_set_register_value(MT6351_PMIC_RG_PWRKEY_RST_TD, 0x03);
	}
#endif
#ifdef CONFIG_ONEKEY_REBOOT_RECOVERY_MODE
	else if (get_boot_mode() == RECOVERY_BOOT) {
		pmic_set_register_value(MT6351_PMIC_RG_PWRKEY_RST_EN, 0x01);
		pmic_set_register_value(MT6351_PMIC_RG_HOMEKEY_RST_EN, 0x00);
		pmic_set_register_value(MT6351_PMIC_RG_PWRKEY_RST_TD, CONFIG_KPD_PMIC_LPRST_TD);
	}
