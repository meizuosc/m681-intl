...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.linelength = 1676,//1680,				//record different mode's linelength
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x3d8c, 0x73); //; Header address high byte
	write_cmos_sensor(0x3d8d, 0xBF); //; Header address low byte
...
*
*************************************************************************/
#define RG_Ratio_Typical (0x256)
#define BG_Ratio_Typical (0x268)
...
//bit[6]:0-no otp info, 1-valid otp info
static int read_otp(struct otp_struct *otp_ptr)
{
	LOG_INF("OV5670 read_otp \n");
	...
	if (addr != 0) {
		...
		(*otp_ptr).prodyction_year = read_cmos_sensor(addr + 2);
		(*otp_ptr).production_month = read_cmos_sensor(addr + 3);
		(*otp_ptr).production_day = read_cmos_sensor(addr + 4);
	} else {
		(*otp_ptr).flag = 0x00;	//no info in OTP
		(*otp_ptr).module_integrator_id = 0;
		(*otp_ptr).lens_id = 0;
		(*otp_ptr).prodyction_year = 0;
		(*otp_ptr).production_month = 0;
		(*otp_ptr).production_day = 0;
	}
	LOG_INF("OV5670 addr = 0x%x, flag = %d, module_integrator_id = %d, lens_id = %d, prodyction_year = %d, production_month = %d, production_day = %d \n",\
					addr, (*otp_ptr).flag, (*otp_ptr).module_integrator_id,(*otp_ptr).lens_id, \
					(*otp_ptr).prodyction_year, (*otp_ptr).production_month, (*otp_ptr).production_day);
	//OTP WB Calibration
	...
	LOG_INF("OV5670 addr = 0x%x, rg_ratio = 0x%x, bg_ratio = 0x%x \n", addr, (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
...
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		LOG_INF("OV5670 R_gain = 0x%x, G_gain = 0x%x, B_gain = 0x%x \n", R_gain, G_gain, B_gain);
		//update sensor WB gain
		/* LOG_INF("OV5670 before apply otp R_gain = 0x%x, G_gain = 0x%x, B_gain = 0x%x \n", \
				(read_cmos_sensor(0x5032) << 8) | (read_cmos_sensor(0x5033) & 0x00FF),\
				(read_cmos_sensor(0x5034) << 8) | (read_cmos_sensor(0x5035) & 0x00FF),\
				(read_cmos_sensor(0x5036) << 8) | (read_cmos_sensor(0x5037) & 0x00FF)); */
		if (R_gain > 0x400) {
		...
		/* LOG_INF("OV5670 after apply otp R_gain = 0x%x, G_gain = 0x%x, B_gain = 0x%x \n", \
				(read_cmos_sensor(0x5032) << 8) | (read_cmos_sensor(0x5033) & 0x00FF),\
				(read_cmos_sensor(0x5034) << 8) | (read_cmos_sensor(0x5035) & 0x00FF),\
				(read_cmos_sensor(0x5036) << 8) | (read_cmos_sensor(0x5037) & 0x00FF)); */
	}
...
static void qunhui_otp_cali(void)
{
	LOG_INF("OV5670 qunhui_otp_cali \n");
    static int qunhui_otp_init_done = 0;
    static struct otp_struct qunhui_otp;
    if (!qunhui_otp_init_done) {
        memset(&qunhui_otp, 0, sizeof(struct otp_struct));
        read_otp(&qunhui_otp);
    }

	LOG_INF("OV5670 qunhui_otp_init_done = %d \n", qunhui_otp_init_done);
    if (qunhui_otp_init_done == 1)
        apply_otp(&qunhui_otp);
    else if (qunhui_otp_init_done != 1)
        qunhui_otp_init_done = 1;
}
