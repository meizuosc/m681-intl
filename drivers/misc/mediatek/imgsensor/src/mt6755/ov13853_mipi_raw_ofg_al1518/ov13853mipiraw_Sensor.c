...
#define PFX "ofilm OV13853_camera_sensor"
...
#define RG_Ratio_Typical (0x121)
#define BG_Ratio_Typical (0x13C)


#define	VENDOR_ID_OFG	0x7
static int vendor_id_ofg = 0x0;

extern void ofg_otp_cali(unsigned char writeid);
extern bool ofg_read_otp_pdaf_data( kal_uint16 addr, BYTE* data, kal_uint32 size);

extern bool ofg_byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data);
extern int ofg_selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13853_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0x571e7117,//0xbde6b5f8,//0xf86cfdf4,        //checksum value for Camera Auto Test
    .pre = {
...
*
*************************************************************************/
struct ofilm_otp_struct {
	u8 module_info_flag;		//[addr:0x0000]0x80:valid
	u8 production_year; 		//[addr:0x0001]
	u8 production_month;		//[addr:0x0002]
	u8 production_day;			//[addr:0x0003]
	u8 module_integrator_id;	//[addr:0x0004]0x07:ofilm
	u8 lens_id; 				//[addr:0x0005]
	u8 vcm_id;					//[addr:0x0006]
	u8 AWB_flag;				//[addr:0x0020]0x80:valid
	u8 AWBdata[28]; 			//[addr:0x0021-0x003C]
	u8 AWBChecksum; 			//[addr:0x003D]Sum(0x0021~0x003C)%255+1
};

static int ofilm_apply_otp(struct ofilm_otp_struct *ofilm_otp_ptr)
{
	int nR_G_gain, nB_G_gain, nG_G_gain;
	int nBase_gain;
	int R_gain, B_gain, G_gain;

	int tempRGain = (ofilm_otp_ptr->AWBdata[1] << 2) | (ofilm_otp_ptr->AWBdata[0]);
	int tempBGain = (ofilm_otp_ptr->AWBdata[3] << 2) | (ofilm_otp_ptr->AWBdata[2]);
	int tempGGain = (ofilm_otp_ptr->AWBdata[5] << 2) | (ofilm_otp_ptr->AWBdata[4]);

	nR_G_gain = (RG_Ratio_Typical * 1000) / tempRGain;
	nB_G_gain = (BG_Ratio_Typical * 1000) / tempBGain;
	nG_G_gain = 1000;
	...
	LOG_INF("nR_G_gain = 0x%x, nB_G_gain = 0x%x, nG_G_gain = 0x%x \n", nR_G_gain, nB_G_gain, nG_G_gain);
	...
	if (R_gain > 0x400) {
		write_cmos_sensor(0x5056, R_gain >> 8);
		write_cmos_sensor(0x5057, R_gain & 0x00FF);
	}
	if (G_gain > 0x400) {
		write_cmos_sensor(0x5058, G_gain >> 8);
		write_cmos_sensor(0x5059, G_gain & 0x00FF);
	} if(B_gain>0x400) {
		write_cmos_sensor(0x505A, B_gain >> 8);
		write_cmos_sensor(0x505B, B_gain & 0x00FF);
	}
...
static int ofilm_read_otp(struct ofilm_otp_struct *ofilm_otp_ptr)
{
	int i, awb_checksum = 0;

	ofg_byteread_cmos_sensor(0xA0, 0x0000, &ofilm_otp_ptr->module_info_flag);
	ofg_byteread_cmos_sensor(0xA0, 0x0001, &ofilm_otp_ptr->production_year);
	ofg_byteread_cmos_sensor(0xA0, 0x0002, &ofilm_otp_ptr->production_month);
	ofg_byteread_cmos_sensor(0xA0, 0x0003, &ofilm_otp_ptr->production_day);
	ofg_byteread_cmos_sensor(0xA0, 0x0004, &ofilm_otp_ptr->module_integrator_id);
	ofg_byteread_cmos_sensor(0xA0, 0x0005, &ofilm_otp_ptr->lens_id);
	ofg_byteread_cmos_sensor(0xA0, 0x0006, &ofilm_otp_ptr->vcm_id);
	ofg_byteread_cmos_sensor(0xA0, 0x0020, &ofilm_otp_ptr->AWB_flag);
	ofg_byteread_cmos_sensor(0xA0, 0x003D, &ofilm_otp_ptr->AWBChecksum);

	if ((ofilm_otp_ptr->module_info_flag == 0x80) && (ofilm_otp_ptr->module_integrator_id == VENDOR_ID_OFG)) {
			vendor_id_ofg = ofilm_otp_ptr->module_integrator_id;
	} else {
		LOG_INF("ofilm otp invalid \n");
		return FALSE;
	}

	ofg_selective_read_region(0x0021, &ofilm_otp_ptr->AWBdata[0], 0xA0, 28);

	LOG_INF("module_info_flag = 0x%x \n", ofilm_otp_ptr->module_info_flag);
    LOG_INF("module_integrator_id = 0x%x \n", ofilm_otp_ptr->module_integrator_id);
    LOG_INF("lens_id = 0x%x \n", ofilm_otp_ptr->lens_id);
    LOG_INF("production_year = 0x%x \n", ofilm_otp_ptr->production_year);
    LOG_INF("production_month = 0x%x \n", ofilm_otp_ptr->production_month);
    LOG_INF("production_day = 0x%x \n", ofilm_otp_ptr->production_day);
    LOG_INF("AWB_flag = 0x%x \n", ofilm_otp_ptr->AWB_flag);
    LOG_INF("AWBChecksum = 0x%x \n", ofilm_otp_ptr->AWBChecksum);

    for (i = 0; i < 28; i++) {
        LOG_INF("AWBdata[%d] = 0x%x \n", i, ofilm_otp_ptr->AWBdata[i]);
    }

	for (i = 0; i < 28; i++)
		awb_checksum += ofilm_otp_ptr->AWBdata[i];
	awb_checksum = (awb_checksum % 0xFF) + 1;

	if (ofilm_otp_ptr->AWBChecksum != awb_checksum)	{
		LOG_INF("ofg otp awb checksum fail \n");
		return FALSE;
	}
	LOG_INF("ofg otp awb checksum success \n");
}
static void ofilm_otp_cali(void)
{
    static int ofilm_otp_init_done = 0;
    static struct ofilm_otp_struct ofilm_otp;
    if (!ofilm_otp_init_done) {
        memset(&ofilm_otp, 0, sizeof(struct ofilm_otp_struct));
        ofilm_read_otp(&ofilm_otp);
    }
    if ((vendor_id_ofg == VENDOR_ID_OFG) && (ofilm_otp_init_done == 1))
        ofilm_apply_otp(&ofilm_otp);
    else if ((vendor_id_ofg == VENDOR_ID_OFG) && (ofilm_otp_init_done != 1))
        ofilm_otp_init_done = 1;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
				...
				ofilm_otp_cali();
				if (vendor_id_ofg == VENDOR_ID_OFG) {
					LOG_INF("[zhangdong ofg] otp valid");
					return ERROR_NONE;
				} else {
					LOG_INF("[zhangdong ofg]Error vendor otp ! SensorId = 0x%x, VendorId = 0x%x \n", *sensor_id, vendor_id_ofg);
    ...
    if (*sensor_id != imgsensor_info.sensor_id || vendor_id_ofg != VENDOR_ID_OFG) {
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            //sensor_id = imgsensor_info.sensor_id;
            if ((sensor_id == imgsensor_info.sensor_id) && (vendor_id_ofg == VENDOR_ID_OFG)) {
				LOG_INF("[zhangdong ofg]correct sensorid and vendorid");
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_PDAF_DATA:
          //add for ov13853 pdaf
			ofg_read_otp_pdaf_data((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
...
UINT32 OV13853_MIPI_RAW_OFG_AL1518_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
