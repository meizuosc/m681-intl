...
static unsigned int g_Sdstatus = 0;//by liunianliang
...
unsigned int sd_debug_zone[HOST_MAX_NUM] = {
    0,
    0,/* 0, xueyun adjust sd debug level */
    0,
    0,
};
...
struct delayed_work sd_recovery;  /* check if write busy timeout*/
static int sd_err_cont = 0;
static int is_sd_rec_working = 0;
unsigned long sd_rec_tmo = 0;
unsigned long sd_rec_timeout = 1000;

static void msdc_timeout_recovery(struct work_struct *work)
{
	printk(KERN_EMERG "[sdrecovery]msdc_timeout_recovery, is_sd_rec_working = %d, sd_err_cont = %d, sd_rec_tmo = %lu \n",
		is_sd_rec_working,
		sd_err_cont,
		sd_rec_tmo);
	if( 1 == is_sd_rec_working )
	{
		//mt_set_gpio_out(GPIO_SDCARD_EXT_LDO_EN_PIN, GPIO_OUT_ZERO);
		//mt_set_gpio_out(GPIO_SDCARD_EXT_LDO_EN_PIN, GPIO_OUT_ONE);
		irq_set_irq_type(cd_irq, IRQ_TYPE_LEVEL_HIGH);
		is_sd_rec_working = 0;
	}
}

static int sd_check_todo_recovery(void)
{
	printk(KERN_EMERG "[sdrecovery]sd_check_todo_recovery, is_sd_rec_working = %d, sd_err_cont = %d, sd_rec_tmo = %lu\n",
		is_sd_rec_working,
		sd_err_cont,
		sd_rec_tmo);
	if ( 0 == is_sd_rec_working )
	{
		//if error, initialies sd_err_cont and sd_rec_tmo.
		if ( 0 == sd_err_cont )
		{
			sd_err_cont++;
			sd_rec_tmo = jiffies + sd_rec_timeout;
		}
		else
		{
			sd_err_cont++;
		...
		///is timeout?
		if (( 0 != sd_rec_tmo ) && (time_after(jiffies, sd_rec_tmo)))
		{
			printk(KERN_EMERG "[sdrecovery]sd_check_todo_recovery, timeout, no need to do recovery.\n");
			sd_err_cont = 0;
			sd_rec_tmo = 0;
			is_sd_rec_working = 0;
			return 0;
		...
		{
			sd_rec_tmo = jiffies + sd_rec_timeout;
			printk(KERN_EMERG "[sdrecovery]sd_check_todo_recovery, not timeout, reinit sd_rec_tmo.sd_rec_tmo = %lu.\n", sd_rec_tmo);
		}
		///is need to recovery?
		if ( 120 <= sd_err_cont )// && (time_before(jiffies, sd_rec_tmo)))
		{
			sd_err_cont = 0;
			schedule_delayed_work(&sd_recovery,msecs_to_jiffies(100));
			is_sd_rec_working = 1;
			printk(KERN_EMERG "[sdrecovery]sd_check_todo_recovery, schedule_delayed_work, is_sd_rec_working = %d, sd_err_cont = %d, sd_rec_tmo = %lu\n",
				is_sd_rec_working,
				sd_err_cont,
				sd_rec_tmo);
			return 0;
...
static irqreturn_t msdc1_eint_handler(void)
{
	...
	/*******chenhuazhen add for sdcard ext ldo start*******/
	int ret = 0;
       if (host->hw->cd_level ^ host->sd_cd_polarity)
	{
           ret=mt_set_gpio_out(GPIO_SDCARD_EXT_LDO_EN_PIN, GPIO_OUT_ONE);
           printk("SD card GPIO_SDCARD_EXT_LDO_EN_PIN set ONE\n");
	 g_Sdstatus = 1;    //add by liunianliang
       } 
	else 
	{
           ret =mt_set_gpio_out(GPIO_SDCARD_EXT_LDO_EN_PIN, GPIO_OUT_ZERO);
           printk("SD card GPIO_SDCARD_EXT_LDO_EN_PIN set ZERO\n");
	 g_Sdstatus = 0;    //add by liunianliang
       }
       if (ret < 0)
           printk("SD card GPIO_SDCARD_EXT_LDO_EN_PIN set Fail\n");
	/*******chenhuazhen add for sdcard ext ldo end*******/
	return IRQ_HANDLED;
}
#endif
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&sd_recovery, msdc_timeout_recovery);
...
//add by liunianliang for show hotplug status 2016.03.14 begin
ssize_t ProcfsSdstatusRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos) {
	printk("*** %s() : status = %d ***\n", __func__,g_Sdstatus);
	...
	ptr = page;
	ptr += sprintf(ptr, "STATUS=%d",g_Sdstatus);
	len = ptr - page;
         if(*pPos >= len) {
                 kfree(page);
                 return 0;
         }
         err = copy_to_user(pBuffer,(char *)page,len);
         *pPos += len;
         if(err) {
		kfree(page);
		return err;
...
static const struct file_operations _gProcSdstatus = {
	.read = ProcfsSdstatusRead,
	.write = NULL,
};

int msdc_sd_status_proc_init(void) {
	static struct proc_dir_entry *_gProcClass = NULL;
	static struct proc_dir_entry *_gProcSD= NULL;
	static struct proc_dir_entry *_gProcStatus = NULL;

	_gProcClass = proc_mkdir("class", NULL);
	_gProcSD = proc_mkdir("sd-hotplug", _gProcClass);
	_gProcStatus = proc_mkdir("status", _gProcSD);

	if (NULL == proc_create("hot-status", 0666, _gProcStatus, &_gProcSdstatus)) {
		pr_err(DRV_NAME ": Can't create proc of sd-hotplug status");
	} else {
		printk("sd-hotplug status create ok !\n");
	}
}
//end
