...
static struct work_struct ofilm_af_work;

static struct workqueue_struct *ofilm_af_work_queue = NULL;
...
static void ofilm_af_work_callback(struct work_struct *work)
{
	u16 val = 300;
	...
	for (val = 300; val >= 100; val -= 20) {
		s4AF_WriteReg(val);
		msleep(20);
		LOG_INF("zhangdong dw9714ofg val = %d\n", val);
	}
...
/*
void ofilm_vcm_smooth_enable(void)
{
	mutex_lock(&af_work_lock);
	if (!af_enable) {}
	af_enable++;
...
*/
void ofilm_vcm_smooth_disable(void)
{
	u16 val = 300;

#if 1
	if (ofilm_af_work_queue) {
		queue_work(ofilm_af_work_queue, &ofilm_af_work);
	} else {
		mutex_lock(&af_work_lock);
		af_enable = 0;
		mutex_unlock(&af_work_lock);
...
//add for lsc mode liyongjian 
	...
	LOG_INF("liyongjian>>ofilm LSC mode successful \n");
//liyongjian	end	
...
static int AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ofilm_af_work_queue = create_singlethread_workqueue("ofilm_af_work");
	if (!ofilm_af_work_queue)
		LOG_INF("[DW9714OFG]Unable to create ofilm_af_work workqueue!\n");

	INIT_WORK(&ofilm_af_work, ofilm_af_work_callback);
...
{
	destroy_workqueue(ofilm_af_work_queue);
