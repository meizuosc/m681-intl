...
#define AF_DRVNAME "DW9714SY"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x19
#define PLATFORM_DRIVER_NAME "lens_actuator_dw9714sy"
#define AF_DRIVER_CLASS_NAME "actuatordrv_dw9714sy"
...
static struct work_struct sunny_af_work;

static struct workqueue_struct *sunny_af_work_queue = NULL;
...
static inline int getAFInfo(__user stDW9714SY_MotorInfo * pstMotorInfo)
{
	stDW9714SY_MotorInfo stMotorInfo;
	...
	if (copy_to_user(pstMotorInfo, &stMotorInfo, sizeof(stDW9714SY_MotorInfo)))
...
#ifdef LensdrvCM3
static inline int getAFMETA(__user stDW9714SY_MotorMETAInfo * pstMotorMETAInfo)
{
	stDW9714SY_MotorMETAInfo stMotorMETAInfo;
	...
	if (copy_to_user(pstMotorMETAInfo, &stMotorMETAInfo, sizeof(stDW9714SY_MotorMETAInfo)))
		LOG_INF("copy to user failed when getting motor information\n");
...
static long AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	switch (a_u4Command) {
	case DW9714SYIOC_G_MOTORINFO:
		i4RetValue = getAFInfo((__user stDW9714SY_MotorInfo *) (a_u4Param));
		break;
#ifdef LensdrvCM3
	case DW9714SYIOC_G_MOTORMETAINFO:
		i4RetValue = getAFMETA((__user stDW9714SY_MotorMETAInfo *) (a_u4Param));
		break;
#endif
	case DW9714SYIOC_T_MOVETO:
		i4RetValue = moveAF(a_u4Param);
		break;

	case DW9714SYIOC_T_SETINFPOS:
		i4RetValue = setAFInf(a_u4Param);
		break;

	case DW9714SYIOC_T_SETMACROPOS:
...
static void sunny_af_work_callback(struct work_struct *work)
{
	u16 val = 300;

	mutex_lock(&af_work_lock);
	for (val = 300; val >= 100; val -= 20) {
		s4AF_WriteReg(val);
		msleep(20);
		LOG_INF("zhangdong dw9714sy val = %d\n", val);
	}
...
/*
void sunny_vcm_smooth_enable(void)
{
	mutex_lock(&af_work_lock);
	if (!af_enable) {}
	af_enable++;
...
*/
void sunny_vcm_smooth_disable(void)
{
	u16 val = 300;

#if 1
	if (sunny_af_work_queue) {
		queue_work(sunny_af_work_queue, &sunny_af_work);
	} else {
		mutex_lock(&af_work_lock);
		af_enable = 0;
		mutex_unlock(&af_work_lock);
...
//add for lsc mode liyongjian 
    
  	int i4RetValue = 0;
  	char puSendCmd0[2] = {0xec,0xa3};
	char puSendCmd1[2] = {0xa1,0x0d};
	char puSendCmd2[2] = {0xf2,0xd8};
	...
	LOG_INF("liyongjian>> shunyu LSC mode successful \n");
//liyongjian	end
...
static int AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sunny_af_work_queue = create_singlethread_workqueue("sunny_af_work");
	if (!sunny_af_work_queue)
		LOG_INF("[DW9714SY]Unable to create sunny_af_work workqueue!\n");
...
static void __exit DW9714SY_i2C_exit(void)
{
	destroy_workqueue(sunny_af_work_queue);
	platform_driver_unregister(&g_stAF_Driver);
}
module_init(DW9714SY_i2C_init);
module_exit(DW9714SY_i2C_exit);

MODULE_DESCRIPTION("DW9714SY lens module driver");
