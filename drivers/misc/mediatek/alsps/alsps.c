...
static ssize_t ps_show_wakeupflag(struct device* dev,
                                 struct device_attribute *attr, char *buf)
{
       return sprintf(buf, "ps_wakeup_flag = %d\n", ps_wakeup_flag); 
}

int ps_get_wakeup_flag(void)
{
	return ps_wakeup_flag;
}
...
DEVICE_ATTR(pswakeup,     		S_IWUSR | S_IRUGO, ps_show_wakeupflag, NULL);
...
int alsps_aal_get_data()
{
	...
	if(ret < 0){
		ALSPS_ERR("cxt->als_data.get_data ret =%d\n", ret);
		return -1;
	}
	return value;
}
