...
int ppm_main_register_policy(struct ppm_policy_data *policy)
{
	...
	policy->req.limit = kzalloc(ppm_main_info.cluster_num * sizeof(*policy->req.limit), GFP_KERNEL);
	if (!policy->req.limit) {
		ppm_err("@%s: fail to allocate memory for policy request!\n", __func__);
		ret = -ENOMEM;
		goto out;
	}
			...
			struct ppm_client_limit *c_limit, struct ppm_cluster_limit *p_limit)
{
	...
	case PPM_POLICY_SYS_BOOST:
	case PPM_POLICY_USER_MODE:
...
int mt_ppm_main(void)
{
	...
	if (memcmp(ppm_main_info.client_req.cpu_limit, ppm_main_info.last_req.cpu_limit,
		ppm_main_info.cluster_num * sizeof(*ppm_main_info.client_req.cpu_limit))) {
		char buf[30 + 30 * NR_CLUSTERS];
		char *ptr = buf;

		/* send request to client */
		for (i = 0; i < NR_PPM_CLIENTS; i++) {
			if (ppm_main_info.client_info[i].limit_cb)
				ppm_main_info.client_info[i].limit_cb(ppm_main_info.client_req);
		}
		...
		for (i = 0; i < ppm_main_info.client_req.cluster_num; i++) {
			ptr += sprintf(ptr, "(%d)(%d)(%d)(%d) ",
				ppm_main_info.client_req.cpu_limit[i].min_cpufreq_idx,
				ppm_main_info.client_req.cpu_limit[i].max_cpufreq_idx,
				ppm_main_info.client_req.cpu_limit[i].min_cpu_core,
				ppm_main_info.client_req.cpu_limit[i].max_cpu_core
			);

			if (ppm_main_info.client_req.cpu_limit[i].has_advise_freq
				|| ppm_main_info.client_req.cpu_limit[i].has_advise_core)
				ptr += sprintf(ptr, "[(%d)(%d)] ",
					ppm_main_info.client_req.cpu_limit[i].advise_cpufreq_idx,
					ppm_main_info.client_req.cpu_limit[i].advise_cpu_core
				);
		}

		ppm_dbg("%s\n", buf);

		memcpy(ppm_main_info.last_req.cpu_limit, ppm_main_info.client_req.cpu_limit,
			ppm_main_info.cluster_num * sizeof(*ppm_main_info.client_req.cpu_limit));
	}
...
static int ppm_main_data_init(void)
{
	...
	/* init cluster info (DVFS table will be updated after DVFS driver registerd) */
