...
static void hwmsen_work_func(struct work_struct *work)
{
	...
	for(idx = 0; idx < MAX_ANDROID_SENSOR_NUM; idx++)
	{
		...
		else
		{
			...
			||(idx == ID_PROXIMITY) ||(idx == ID_PROXIMITY_WAKEUP) || (idx == ID_TEMPRERATURE))
			{
	...
	if(enable_again == true)
	{
		...
		for(idx = 0; idx <= MAX_ANDROID_SENSOR_NUM; idx++)
	    {
		   ...
		   if(ID_PROXIMITY==idx || ID_LIGHT==idx || ID_PRESSURE==idx || ID_PROXIMITY_WAKEUP == idx)
	       {
	          if(SENSOR_INVALID_VALUE == obj_data.sensors_data[idx].values[0])
	          {
	       	   event_type &= ~(1LL << idx);
	...
	if((event_type&(1LL << ID_PROXIMITY_WAKEUP))&& SENSOR_INVALID_VALUE == obj_data.sensors_data[ID_PROXIMITY_WAKEUP].values[0])
	{
	    event_type &= ~(1LL << ID_PROXIMITY_WAKEUP);   
		//HWM_LOG("remove ps event!!!!!!!!!!!\n");
	}
