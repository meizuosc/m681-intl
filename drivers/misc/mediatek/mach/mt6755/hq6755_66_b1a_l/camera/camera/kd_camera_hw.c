...
PowerUp PowerOnList = {
	{
		...
		{SENSOR_DRVNAME_OV13853_MIPI_RAW_OFG_AL1518,
			{
				{SensorMCLK, Vol_High, 0},
				{AVDD, Vol_2800, 0},
				{DOVDD, Vol_1800, 0},
		...
		{SENSOR_DRVNAME_OV13853_MIPI_RAW_SY_AL1518,
			{
				{SensorMCLK, Vol_High, 0},
				{AVDD, Vol_2800, 0},
				{DOVDD, Vol_1800, 0},
...
extern void sunny_vcm_smooth_disable(void);
extern int sunny_vcm_opened;
extern void ofilm_vcm_smooth_disable(void);
extern int ofilm_vcm_opened;
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		if (sunny_vcm_opened) {
			sunny_vcm_smooth_disable();
			sunny_vcm_opened = 0;
		}
		if (ofilm_vcm_opened) {
			ofilm_vcm_smooth_disable();
			ofilm_vcm_opened = 0;
		}
#if 0
		PK_DBG("[CAMERA SENSOR] Skip AFVDD setting\n");
