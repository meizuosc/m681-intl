...
static int vibe_state,vibe_time;
...
{
	printk("[vibrator] vibr_Enable: vibe_state = %d\n",vibe_state);
	if (!ldo_state) {
		vibr_Enable_HW();
		ldo_state = 1;
	}
...
{
	printk("[vibrator] vibr_Disable: state = %d\n",vibe_state);
	if (ldo_state) {
		vibr_Disable_HW();
		ldo_state = 0;
	}
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	ret = queue_work(vibrator_queue, &vibrator_work);
...
static enum hrtimer_restart vibrator_timer_func(struct hrtimer *timer)
{
	...
	ret = queue_work(vibrator_queue, &vibrator_work);
	printk("[vibrator] hrtimer_restart vibrator_timer_func queue_work = %d\n",ret);
