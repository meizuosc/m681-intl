...
int ofg_selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size);
...
//      CAM_CALDB("selective_read_byte\n");
	...
	if(iReadRegI2C(&offset, 1, (u8*)data, 1, i2c_id+(page<<1))<0) {
		CAM_CALERR("[CAM_CAL_OFG] fail selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x",
...
*/
bool ofg_byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data)
{
	...
	if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
		CAM_CALERR("[CAM_CAL_OFG] fail ov13853ofg_byteread_cmos_sensor addr =0x%x, data = 0x%x \n", addr, *data);
		return false;
	}
	/*CAM_CALDB("selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr,
	*data,page,offset);*/
	CAM_CALDB("[CAM_CAL_OFG] ov13853ofg_cmos_sensor addr =0x%x, data = 0x%x \n", addr, *data);
	return true;
...
int ofg_selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	/*kdSetI2CSpeed(EEPROM_I2C_SPEED);*/
	int ret = 0;
	CAM_CALDB("Before ofg_byteread_cmos_sensor curAddr =%x count=%d buffData=%x\n", curAddr,
	size - size_to_read, *buff);
	while (size_to_read > 0) {
		/*if(selective_read_byte(addr,(u8*)buff,OV13853OFG_DEVICE_ID)){*/
		if (ofg_byteread_cmos_sensor(OV13853OFG_DEVICE_ID, curAddr, buff)) {
			CAM_CALDB("after ofg_byteread_cmos_sensor curAddr =%x count=%d buffData=%x\n", curAddr,
			size - size_to_read, *buff);
			curAddr += 1;
			buff += 1;
			size_to_read -= 1;
	...
	CAM_CALDB("ofg_selective_read_region addr =%x size %d readSize = %d\n", addr, size, ret);
	return ret;
}
...
{
	CAM_CALDB("[CAM_CAL_OFG] not implemented!");
	return 0;
}
...
static long ov13853ofg_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret;
	COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
	stCAM_CAL_INFO_STRUCT __user *data;
	int err;
	CAM_CALDB("[CAMERA SENSOR] ov13853ofg_Ioctl_Compat,%p %p %x ioc size %d\n", filp->f_op ,
	...
	case COMPAT_CAM_CALIOC_G_READ: {
		...
		if (err != 0)
			CAM_CALERR("[CAM_CAL_OFG] compat_put_acdk_sensor_getinfo_struct failed\n");
		return ret;
	}
	default:
		return -ENOIOCTLCMD;
...
#endif
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff , (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				...
				kfree(pBuff);
				CAM_CALDB("[CAM_CAL_OFG] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	if (NULL == pu1Params) {
		kfree(pBuff);
		CAM_CALDB("[CAM_CAL_OFG] ioctl allocate mem failed\n");
		return -ENOMEM;
	}
	...
	case CAM_CALIOC_S_WRITE:
		CAM_CALDB("[CAM_CAL_OFG] Write CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		CAM_CALDB("[CAM_CAL_OFG] Read CMD\n");

#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		CAM_CALDB("[CAM_CAL_OFG] offset %d\n", ptempbuf->u4Offset);
		CAM_CALDB("[CAM_CAL_OFG] length %d\n", ptempbuf->u4Length);
		/**pu1Params = 0;*/
		i4RetValue = ofg_selective_read_region(ptempbuf->u4Offset, pu1Params, OV13853OFG_DEVICE_ID, ptempbuf->u4Length);
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv2);
		if (ktv2.tv_sec > ktv1.tv_sec)
			TimeIntervalUS = ktv1.tv_usec + 1000000 - ktv2.tv_usec;
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		CAM_CALDB("[CAM_CAL_OFG] to user length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[CAM_CAL_OFG] to user  Working buffer address 0x%p\n", pu1Params);
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pu1Params , ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pu1Params);
			CAM_CALDB("[CAM_CAL_OFG] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
{
	CAM_CALDB("[CAM_CAL_OFG] CAM_CAL_Open\n");
	spin_lock(&g_CAM_CALLock);
	if (g_u4Opened) {
		spin_unlock(&g_CAM_CALLock);
		CAM_CALDB("[CAM_CAL_OFG] Opened, return -EBUSY\n");
		return -EBUSY;
	} else {
		g_u4Opened = 1;
...
static const struct file_operations g_stCAM_CAL_fops = {
	...
	/*.ioctl = CAM_CAL_Ioctl*/
#ifdef CONFIG_COMPAT
	.compat_ioctl = ov13853ofg_Ioctl_Compat,
#endif
...
{
	CAM_CALDB("[CAM_CAL_OFG] RegisterCAM_CALCharDrv Start\n");

	struct device *CAM_CAL_device = NULL;

#if CAM_CAL_DYNAMIC_ALLOCATE_DEVNO
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[CAM_CAL_OFG] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno , 1 , CAM_CAL_DRVNAME)) {
		CAM_CALDB("[CAM_CAL_OFG] Register device no failed\n");
	...
	if (NULL == g_pCAM_CAL_CharDrv) {
		...
		CAM_CALDB("[CAM_CAL_OFG] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALDB("[CAM_CAL_OFG] Attatch file operation failed\n");
	...
	if (IS_ERR(CAM_CAL_class)) {
		...
		CAM_CALDB("[CAM_CAL_OFG] Unable to create class, err = %d\n", ret);
		return ret;
	}
	CAM_CAL_device = device_create(CAM_CAL_class, NULL, g_CAM_CALdevno, NULL, CAM_CAL_DRVNAME);
	CAM_CALDB("[CAM_CAL_OFG] RegisterCAM_CALCharDrv End\n");
...
{
	int i4RetValue = 0;
	CAM_CALDB("[CAM_CAL_OFG] CAM_CAL_i2c_probe Start!\n");
	/*    spin_lock_init(&g_CAM_CALLock);*/

	/*get sensor i2c client*/
	spin_lock(&g_CAM_CALLock); /*for SMP*/
	g_pstI2Cclient = client;
	g_pstI2Cclient->addr = OV13853OFG_DEVICE_ID >> 1;
	spin_unlock(&g_CAM_CALLock); /* for SMP*/

	CAM_CALDB("[CAM_CAL_OFG] g_pstI2Cclient->addr = 0x%x\n", g_pstI2Cclient->addr);
	/*Register char driver*/
	i4RetValue = RegisterCAM_CALCharDrv();

	if (i4RetValue) {
		CAM_CALDB("[CAM_CAL_OFG] register char device failed!\n");
		return i4RetValue;
	}

	CAM_CALDB("[CAM_CAL_OFG] CAM_CAL_i2c_probe End!\n");
	return 0;
}
...
{
	CAM_CALDB("[CAM_CAL_OFG] CAM_CAL_probe start!\n");
...
static int __init CAM_CAL_OFG_i2C_init(void)
{
	CAM_CALDB("[CAM_CAL_OFG] CAM_CAL_i2C_init Start!\n");
	i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALDB("[CAM_CAL_OFG] failed to register CAM_CAL driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALDB("[CAM_CAL_OFG] failed to register CAM_CAL driver, 2nd time\n");
		return -ENODEV;
	}
	CAM_CALDB("[CAM_CAL_OFG] CAM_CAL_i2C_init End!\n");
	return 0;
}

static void __exit CAM_CAL_OFG_i2C_exit(void)
{
	platform_driver_unregister(&g_stCAM_CAL_Driver);
}

module_init(CAM_CAL_OFG_i2C_init);
module_exit(CAM_CAL_OFG_i2C_exit);

MODULE_DESCRIPTION("OV13853OFG CAM_CAL driver");
